J_segment.F <- function(Feat, SQuali, SQualiN){
  ...
  J_segment <- data.frame("Location" = "J_segment", "Qualifier" = gsub(".*J_segment +([^.]+)\"*", "\\1", Feat[1], perl = T), stringsAsFactors = F)
  for(i in 2:length(Feat)){
    for(j in 1:length(Item)){
      if(length(grep(Item[j], Feat[i], perl = T)) == 1){
        J_segment <- rbind(J_segment, c(ItemN[j],  gsub(".*=([^.]+)\"*", "\\1", Feat[i])))
        if((length(grep("\\\\\"$|\"\\\", $|\\d$", Feat[i])) == 0 & i != length(Feat)) == T){
          t <- i+1
          while(length(grep("\\\\\"$|\"\\\", $|\\d$", Feat[t])) == 0 && t <= length(Feat)){
            J_segment[dim(J_segment)[1],2] <- paste(J_segment[dim(J_segment)[1],2], gsub("\\s", " ", Feat[t]), sep = " ")
            t <- t+1
          }
    ...
    for(k in 1:length(SQuali)){
      if(length(grep(SQuali[k], Feat[i], perl = T)) == 1){
        J_segment <- rbind(J_segment, c(SQuali[k], SQualiN[k]))
      }
    }
  }
